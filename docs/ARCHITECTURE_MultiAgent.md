...
- [ADR-0001: Agent Framework Selection](./ADR-0001-agent-framework-selection.md) — why this architecture is built with plain TypeScript Cloud Functions + the Anthropic SDK rather than an agent framework (Pydantic AI / Google ADK / Claude Managed Agents), and the upgrade path when a step becomes genuinely agentic.
- [PRODUCT_CONSTITUTION.md](./PRODUCT_CONSTITUTION.md) — the five first principles this architecture must obey.
- [PRODUCT_Autonomy_Ladder.md](./PRODUCT_Autonomy_Ladder.md) — the L0–L4 model the Autonomy Governor Agent implements.
